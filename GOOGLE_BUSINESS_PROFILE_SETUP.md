# Google Business Profile Setup & Optimization Guide
**Time Estimate: 1-2 hours** | **Impact: Major local SEO ranking factor**

This guide will walk you through claiming, verifying, and optimizing your Google Business Profile (GBP) for Bokkie Cleaning Services.

---

## 📋 Pre-Setup Checklist

Before you begin, gather the following information:

- [ ] Business name: **Bokkie Cleaning Services**
- [ ] Business address: **39 Harvey Road, Claremont, Western Cape, 7708, South Africa**
- [ ] Phone number: **+27871535250**
- [ ] Email: **support@bokkie.com**
- [ ] Website: **https://bokkie.co.za**
- [ ] Business hours (24/7 - Monday to Sunday, 00:00-23:59)
- [ ] Business category: Cleaning Service / Professional Cleaning Services
- [ ] Service areas: Cape Town, Sea Point, Camps Bay, Claremont, Green Point, V&A Waterfront, Constantia
- [ ] At least 10-15 high-quality photos (see Photo Requirements below)
- [ ] Business description (200-750 characters)

---

## 🚀 Step 1: Claim Your Google Business Profile

### Option A: If a profile already exists
1. Go to [Google Business Profile](https://www.google.com/business/)
2. Search for "Bokkie Cleaning Services" or your business address
3. If you find your business, click **"Claim this business"**
4. Follow the verification process

### Option B: Create a new profile
1. Go to [Google Business Profile Manager](https://www.google.com/business/)
2. Click **"Manage now"** or **"Get started"**
3. Sign in with your Google account (use a business email if possible)
4. Enter your business name: **Bokkie Cleaning Services**
5. Select your business category:
   - Primary: **"Cleaning Service"** or **"Professional Cleaning Service"**
   - Secondary (if available): **"House Cleaning Service"**

---

## 📍 Step 2: Add Complete Business Information

### Basic Information
- **Business Name**: Bokkie Cleaning Services
- **Business Category**: Cleaning Service / Professional Cleaning Service
- **Address**: 
  - Street: 39 Harvey Road
  - City: Claremont
  - State/Province: Western Cape
  - Postal Code: 7708
  - Country: South Africa
- **Phone Number**: +27871535250
- **Website**: https://bokkie.co.za
- **Email**: support@bokkie.com

### Business Hours
Set your hours to reflect 24/7 availability:
- **Monday**: Open 24 hours (or 00:00 - 23:59)
- **Tuesday**: Open 24 hours
- **Wednesday**: Open 24 hours
- **Thursday**: Open 24 hours
- **Friday**: Open 24 hours
- **Saturday**: Open 24 hours
- **Sunday**: Open 24 hours

**Note**: If Google doesn't allow 24-hour settings, use 00:00-23:59 for each day.

### Service Areas
Add all service areas:
- Cape Town
- Sea Point
- Camps Bay
- Claremont
- Green Point
- V&A Waterfront
- Constantia

---

## 📝 Step 3: Write an Optimized Business Description

**Character limit: 200-750 characters**

### Recommended Description:
```
Bokkie Cleaning Services is a professional cleaning company serving Cape Town and surrounding areas including Sea Point, Camps Bay, Claremont, Green Point, V&A Waterfront, and Constantia. We offer comprehensive cleaning services including regular cleaning, deep cleaning, move-in/out cleaning, Airbnb cleaning, office cleaning, apartment cleaning, window cleaning, and home maintenance. Our experienced team uses eco-friendly products and provides 24/7 service with a 100% satisfaction guarantee. Book your cleaning service online at bokkie.co.za or call +27871535250.
```

**SEO Tips for Description:**
- Include primary keyword: "cleaning services Cape Town"
- Mention service areas
- Include key services
- Add call-to-action (booking/phone)
- Include website URL
- Mention unique selling points (eco-friendly, 24/7, satisfaction guarantee)

---

## 📸 Step 4: Add High-Quality Photos

### Photo Requirements:
- **Minimum**: 10-15 photos
- **Format**: JPG or PNG
- **Size**: At least 720x720 pixels (higher resolution preferred)
- **Quality**: High-quality, well-lit, professional photos

### Photo Types to Include:

#### 1. Logo (Required)
- Your business logo
- Square format (1:1 ratio)
- High resolution
- Clear and professional

#### 2. Cover Photo (Required)
- Main business photo
- 16:9 ratio recommended
- Shows your team, vehicles, or office
- Professional and inviting

#### 3. Interior Photos (5-10 photos)
- Clean, organized workspace
- Before/after cleaning shots
- Team members working
- Equipment and supplies
- Office/workspace

#### 4. Exterior Photos (2-3 photos)
- Business location/storefront (if applicable)
- Company vehicles with branding
- Team at work locations

#### 5. Team Photos (3-5 photos)
- Professional team photos
- Cleaners in action
- Uniformed staff
- Happy team members

#### 6. Service Photos (5-10 photos)
- Before/after cleaning comparisons
- Different cleaning services in action
- Clean, organized spaces
- Eco-friendly products

### Photo Optimization Tips:
- Use descriptive filenames: `bokkie-cleaning-team-cape-town.jpg`
- Add alt text when uploading (if option available)
- Update photos regularly (monthly)
- Show diversity in services and locations
- Include photos with people (more engaging)

---

## ⭐ Step 5: Enable and Manage Reviews

### Enable Reviews
1. Go to your GBP dashboard
2. Navigate to **"Reviews"** section
3. Ensure reviews are enabled (they should be by default)

### Review Management Strategy:
- **Respond to ALL reviews** (positive and negative)
- **Response time**: Within 24-48 hours
- **Response length**: 2-4 sentences
- **Tone**: Professional, friendly, and appreciative

### Sample Review Response Templates:

#### For 5-Star Reviews:
```
Thank you so much, [Name]! We're thrilled to hear about your positive experience with Bokkie Cleaning Services. Your feedback means the world to us and motivates our team to continue delivering exceptional service. We look forward to serving you again!
```

#### For 4-Star Reviews:
```
Thank you for your feedback, [Name]! We appreciate you taking the time to share your experience. We're always working to improve, and your input helps us serve you better. We hope to earn that 5th star on your next visit!
```

#### For Negative Reviews:
```
Hi [Name], we're sorry to hear about your experience. This doesn't reflect our usual standard of service. We'd like to make this right. Please contact us directly at +27871535250 or support@bokkie.com so we can resolve this issue. Thank you for bringing this to our attention.
```

### Encourage Reviews:
- Add review link to email signatures
- Include review request in post-service follow-ups
- Add review CTA on website
- Create QR codes linking to review page
- Ask satisfied customers directly

---

## 🔗 Step 6: Link to Website

### Add Website Link:
1. In your GBP dashboard, go to **"Info"** section
2. Find **"Website"** field
3. Enter: **https://bokkie.co.za**
4. Save changes

### Verify Website Connection:
- Ensure your website has consistent NAP (Name, Address, Phone) information
- Add GBP link to your website footer
- Consider adding a "Find us on Google" badge

---

## 🎯 Step 7: Add Services

### Add All Cleaning Services:
1. Go to **"Services"** section in GBP dashboard
2. Add each service with description:

- **Regular Cleaning**
  - Description: Ongoing maintenance cleaning to keep your home fresh and organized

- **Deep Cleaning**
  - Description: Thorough cleaning that addresses every corner and surface

- **Move In/Out Cleaning**
  - Description: Comprehensive cleaning for property transitions

- **Airbnb Cleaning**
  - Description: Professional turnover cleaning for short-term rentals

- **Office Cleaning**
  - Description: Commercial cleaning services to maintain a clean workspace

- **Apartment Cleaning**
  - Description: Specialized services tailored for apartments and condos

- **Window Cleaning**
  - Description: Professional window cleaning for streak-free results

- **Home Maintenance**
  - Description: Comprehensive services to keep your home in perfect condition

---

## ✅ Step 8: Verification Process

### Verification Methods (Google will choose):
1. **Postcard Verification** (Most Common)
   - Google sends a postcard to your business address
   - Contains a verification code
   - Arrives in 5-7 business days
   - Enter code in GBP dashboard

2. **Phone Verification**
   - Google calls your business number
   - Provides verification code
   - Enter code immediately

3. **Email Verification**
   - Google sends email with verification link
   - Click link to verify

4. **Video Verification** (For some businesses)
   - Live video call with Google representative
   - Show your business location and operations

### After Verification:
- Your profile will be live on Google Search and Maps
- You can start receiving and responding to reviews
- Your business will appear in local search results

---

## 🎨 Step 9: Additional Optimizations

### Add Attributes:
- ✅ **Online estimates** (if you offer online quotes)
- ✅ **Online appointments** (if you have booking system)
- ✅ **Eco-friendly** (if applicable)
- ✅ **Wheelchair accessible** (if applicable)
- ✅ **Women-led** (if applicable)
- ✅ **LGBTQ+ friendly** (if applicable)

### Add Products/Services Menu:
- Create a menu of your services
- Include pricing if possible
- Add service descriptions
- Link to booking pages

### Post Regular Updates:
- **Posts**: Share updates, offers, tips (weekly)
- **Photos**: Add new photos monthly
- **Events**: Promote special offers or events
- **Updates**: Share business news and achievements

### Q&A Section:
- Monitor and answer customer questions
- Add frequently asked questions proactively
- Respond quickly to new questions

---

## 📊 Step 10: Track Performance

### Key Metrics to Monitor:
1. **Profile Views**: How many people view your profile
2. **Search Queries**: What people search to find you
3. **Direction Requests**: How many request directions
4. **Phone Calls**: Calls from GBP listing
5. **Website Clicks**: Clicks to your website
6. **Review Count**: Total number of reviews
7. **Average Rating**: Your star rating

### Access Insights:
1. Go to GBP dashboard
2. Click **"Insights"** tab
3. Review weekly/monthly reports
4. Track trends and optimize accordingly

---

## 🔄 Step 11: Ongoing Maintenance

### Weekly Tasks:
- [ ] Respond to new reviews (within 24-48 hours)
- [ ] Answer new Q&A questions
- [ ] Post an update or offer

### Monthly Tasks:
- [ ] Add new photos (2-3 photos)
- [ ] Review and update business hours if needed
- [ ] Check and update service areas
- [ ] Review insights and adjust strategy
- [ ] Update posts and offers

### Quarterly Tasks:
- [ ] Audit all business information for accuracy
- [ ] Update business description if needed
- [ ] Review and optimize attributes
- [ ] Analyze competitor profiles
- [ ] Update service menu/pricing

---

## 🚨 Common Issues & Solutions

### Issue: Can't find my business to claim
**Solution**: 
- Search with different variations of your business name
- Search by address only
- Create a new profile if business doesn't exist

### Issue: Verification postcard not arriving
**Solution**:
- Wait 14 business days
- Check address is correct
- Contact Google Business Profile support
- Request new verification code

### Issue: Duplicate listings
**Solution**:
- Claim all duplicate listings
- Mark duplicates for removal
- Keep the most complete listing active
- Contact Google support if needed

### Issue: Reviews not showing
**Solution**:
- Reviews may take 24-48 hours to appear
- Ensure reviews comply with Google's policies
- Check if reviews are being filtered
- Contact Google support if reviews disappear

### Issue: Low visibility in search
**Solution**:
- Complete all profile sections (100% complete)
- Get more reviews (aim for 20+)
- Post regularly (weekly)
- Add more photos
- Ensure NAP consistency across web

---

## 📱 Mobile Optimization

### Mobile App:
- Download **Google Business Profile** app (iOS/Android)
- Manage your profile on the go
- Respond to reviews instantly
- Post updates from mobile
- View insights anytime

### Mobile Features:
- Quick responses to reviews
- Photo uploads
- Post creation
- Q&A management
- Customer messaging

---

## 🎯 SEO Benefits Checklist

After completing this setup, you should see:

- [ ] Business appears in Google Maps
- [ ] Business shows in local search results (Local Pack)
- [ ] Increased visibility for "cleaning services Cape Town" searches
- [ ] More website clicks from Google
- [ ] More phone calls from listings
- [ ] Better local ranking signals
- [ ] Improved trust and credibility
- [ ] Higher conversion rates

---

## 📞 Next Steps After Setup

1. **Update Website**: Add GBP link to website footer
2. **Update Structured Data**: Add GBP URL to schema (if applicable)
3. **Email Signature**: Add "Find us on Google" link
4. **Social Media**: Share your GBP link on social profiles
5. **Email Campaigns**: Ask customers to leave reviews
6. **Monitor**: Set up weekly review of insights
7. **Optimize**: Continuously improve based on data

---

## 🔗 Useful Links

- [Google Business Profile](https://www.google.com/business/)
- [Google Business Profile Help](https://support.google.com/business/)
- [Google Business Profile App](https://www.google.com/business/how-it-works/)
- [Local SEO Guide](https://support.google.com/business/answer/3038177)

---

## ✅ Final Checklist

Before considering your GBP setup complete:

- [ ] Business profile claimed and verified
- [ ] All business information complete (100%)
- [ ] Business hours set correctly
- [ ] Service areas added
- [ ] Business description optimized (200-750 chars)
- [ ] Logo uploaded
- [ ] Cover photo uploaded
- [ ] 10+ photos added
- [ ] Website linked
- [ ] All services listed
- [ ] Reviews enabled
- [ ] Attributes added
- [ ] First post created
- [ ] Q&A section monitored
- [ ] Mobile app downloaded
- [ ] Insights dashboard reviewed

---

**Estimated Completion Time**: 1-2 hours
**Expected Impact**: Major improvement in local SEO rankings
**Priority**: 🔴 CRITICAL - Do this immediately

---

*Last Updated: 2024*
*For Bokkie Cleaning Services - Cape Town, South Africa*











